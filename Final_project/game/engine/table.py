...
class Table:
    ...
    def next_active_player_pos(self, start_pos):
        return self.__find_entitled_player_pos(
            start_pos, lambda player: player.is_active() and player.stack != 0
        )

    def next_ask_waiting_player_pos(self, start_pos):
        return self.__find_entitled_player_pos(
            start_pos, lambda player: player.is_waiting_ask()
        )
    ...
    def __find_entitled_player_pos(self, start_pos, check_method):
        ...
        return (
            ...
            if match_player == -1
            else players.index(match_player)
        )
