...
def visualize_game_start(game_info, uuid=None):
    ...
    ls.append(
        _visualize_item("small blind = %s" % game_info["rule"]["small_blind_amount"])
    )
    if len(game_info["rule"]["blind_structure"]) != 0:
        ...
        for round_count in game_info["rule"]["blind_structure"]:
            ...
            ls.append(
                _visualize_sub_item(
                    "after %d round : ante = %s, sb_amount = %s"
...
def visualize_declare_action(valid_actions, hole_card, round_state, uuid=None):
    ...
    ls.append(
        _visualize_item(
            "%s: %s" % (valid_actions[1]["action"], valid_actions[1]["amount"])
        )
    )
    ls.append(
        _visualize_item(
...
def _fetch_player_name(uuid, rs):
    if uuid not in [p["uuid"] for p in rs["seats"]]:
        raise Exception(
            "player of uuid = [ %s ] not found in round state => %s" % (uuid, rs)
        )
...
def visualize_round_state(rs):
    ls = []
    ls.append(
        _visualize_item("dealer btn : %s" % rs["seats"][rs["dealer_btn"]]["name"])
    )
    ...
    for street, histories in sorted(rs["action_histories"].items(), key=sort_key):
        if len(histories) != 0:
            ...
            for history in histories:
                ...
                summary["player"] = "%s (uuid=%s)" % (
                    _fetch_player_name(uuid, rs),
                    uuid,
                )
...
def _is_next_player(player, rs):
    return (
        rs
        and not isinstance(rs["next_player"], str)
